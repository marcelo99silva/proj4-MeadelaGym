...
<!-- <v-btn medium elevation="15" style="float:rigth; background-color: #F7F3E9">Adicionar parceiro</v-btn> -->
    ...
    <tr class="rr" v-for="x in parceirosBd" v-bind:key="x.idFotoParceiro">
        <td class="nomeBD">{{x.nome}}</td>
         <td border=3 height=100 width=100> <img :src="getImgUrl(x.foto)" class="imagemBD" >  </td>
         <td width="100" class="removerTD"> <button class="editarBD" @click="abrirModalUpdate(x.nome, x.foto, x.idFotoParceiro )" >Editar</button>
           <button class="removerBD" @click="removerParceiro(x.idFotoParceiro , x.foto)">Remover</button> </td>
    </tr>
</table>
      </div>

  <v-btn medium elevation="15" @click="myModel=true" value="Add" style="float:right; background-color: #F7F3E9">Adicionar parceiro</v-btn>
    </v-sheet>
          ...
          <div class="modal-content">
            <div class="modal-header">
              ...
              <div class="modal-body">
                 <h4 class="tituloModal">Inserir Parceiro</h4>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header">
              ...
              <!-- <h4 class="modal-title">titulo</h4> -->
              <div class="modal-body">
                 <h4 class="tituloModal">Editar Parceiro</h4>
                <div class="form-group">
...
export default {
  ...
  mounted(){
    ...
    .then((response) =>{
      ...
      this.parceirosBd = response.data;
  ...
  methods: {
    ...
    abrirModalUpdate(nome,link, imagem, id){
    ...
    getImgUrl(pic) {
      return '/assets/gym/parceiros/' + pic
},
  ...
  if(this.fileEmCondicoes && this.nome.length > 0){
      ...
      // try{
    //  await 
    //  axios.post("http://meadelagym.pt/projeto4dev/api/imagem",formData);

      axios({
        method: "post",
        url: "http://meadelagym.pt/projeto4dev/api/imagem",
        data: formData,
    ...
    removerParceiro(idRecebido , nomeRecebido){

      axios({
        method: "post",
        url: "http://meadelagym.pt/projeto4dev/api/deleteParceiros",

         data: {
          idParceiro : idRecebido,
          nome : nomeRecebido
        }
  ...
  if(this.fileEmCondicoes && this.nome.length > 0){
    // try{
    //  await
      // axios.post("http://meadelagym.pt/projeto4dev/api/updateParceiros",formData);

      axios({
        method: "post",
        url: "http://meadelagym.pt/projeto4dev/api/updateParceiros",
        data: formData,
        headers: { "Content-Type": "multipart/form-data" },
      })
