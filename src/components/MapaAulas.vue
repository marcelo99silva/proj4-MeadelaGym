...
<!-- <v-btn medium elevation="15" style="float:rigth; background-color: #F7F3E9">Adicionar parceiro</v-btn> -->
    ...
    <!-- <div  style="margin-top:25px">
     ...
     <h6 class="tituloMeadela">Descrição mapa aulas":</h6>



        <div class="infoEmpresaMeadela">

        <textarea id="w3review" name="w3review" rows="5" cols="80" style="border: 1px solid black" v-model="miniSobre"> </textarea>
        </div>
    ...
    <tr class="rr" v-for="x in mapaBD" v-bind:key="x.idParceiroBD">
        <!-- <td colspan = 3 class="nomeBD">{{x.nome}}</td> -->
        <td colspan = 3 class="nomeBD2">{{x.descricao}}</td> 
        <!-- <td colspan = 3 class="nomeBD">{{x.nome}}</td> -->
         ...
         <button class="removerBD" @click="removerServico(x.idMapa, x.foto)">Remover</button> </td>
    </tr>
</table>
      </div>
...
export default {
  ...
  if(this.fileEmCondicoes){
     ...
     axios({
        method: "post",
        url: "http://meadelagym.pt/projeto4dev/api/mapaaulas",
        data: formData,
...
// this.myModel = false;
    ...
    removerServico(idRecebido , nomeRecebido){
      ...
      axios({
        method: "post",
        url: "http://meadelagym.pt/projeto4dev/api/deleteMapa",
  ...
  mounted(){
    ...
    .then((response) =>{
        this.mapaBD = response.data;
        // console.log(this.mapaBD.length);
